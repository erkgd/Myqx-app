...
class UnaffiliatedProfileService extends ChangeNotifier {
  ...
  /// Obtiene recomendaciones de perfiles
  ///
  /// [limit] - Número máximo de recomendaciones a obtener
  /// Retorna lista de IDs de usuarios recomendados o lista vacía si hay error
  Future<List<String>> getProfileRecommendations({int limit = 5}) async {
    try {
      debugPrint('[DEBUG] Solicitando recomendaciones de perfiles (límite: $limit)');
      
      final response = await _apiClient.get(
        '/api/profile/recommendations?limit=$limit',
        requiresAuth: true,
      );
      
      if (response != null && response['recommendations'] != null) {
        final List<dynamic> rawRecommendations = response['recommendations'] as List<dynamic>;
        final List<String> recommendations = rawRecommendations
            .map((item) => item['userId']?.toString() ?? '')
            .where((id) => id.isNotEmpty)
            .toList();
        
        debugPrint('[DEBUG] Recomendaciones obtenidas desde API: ${recommendations.length}');
        
        // Comenzar a precargar estos perfiles en segundo plano
        if (recommendations.isNotEmpty) {
          preloadPopularProfiles(recommendations);
        }
      ...
      debugPrint('[ERROR] Error obteniendo recomendaciones de perfiles: ${e.toString()}');
      return [];
    }
  }
}
